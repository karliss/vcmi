...
bool VCAI::moveHeroToTile(int3 dst, HeroPtr h)
{
	...
	else
	{
		...
		for(; i > 0; i--)
		{
			...
			bool isConnected = CGTeleport::isConnected(nextObjectTop, getObj(path.nodes[i-2].coord, false));
			bool isNextObjectTeleport = CGTeleport::isTeleport(nextObjectTop);
			// Check there is node after next one; otherwise transit is pointless
			if((i-2 >= 0) && (isConnected || isNextObjectTeleport))
			{
